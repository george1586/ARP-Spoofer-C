...
int setup_dns_redirect(void) {
    ...
    return system("iptables -t nat -A PREROUTING -p udp --dport 53 -j REDIRECT --to-port 53 2>/dev/null || "
...
int cleanup_dns_redirect(void) {
    ...
    return system("iptables -t nat -D PREROUTING -p udp --dport 53 -j REDIRECT --to-port 53 2>/dev/null || "
                  "/sbin/iptables -t nat -D PREROUTING -p udp --dport 53 -j REDIRECT --to-port 53 2>/dev/null || "
                  "/usr/sbin/iptables -t nat -D PREROUTING -p udp --dport 53 -j REDIRECT --to-port 53");
}
