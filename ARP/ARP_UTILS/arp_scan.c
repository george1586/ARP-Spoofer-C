...
struct Victim* scan_network(unsigned char* gateway_ip, int* out_count) {
    ...
    *out_count = 0;
    ...
    printf("[*] Scanning network from %d.%d.%d.%d to %d.%d.%d.%d\n",
        (net_int >> 24) & 0xFF, (net_int >> 16) & 0xFF, (net_int >> 8) & 0xFF, net_int & 0xFF,
        (bcast_int >> 24) & 0xFF, (bcast_int >> 16) & 0xFF, (bcast_int >> 8) & 0xFF, bcast_int & 0xFF);
    ...
    for (uint32_t target = net_int + 1; target < bcast_int; target++) {
        ...
        if (memcmp(&t_ip, my_ip, 4) == 0) continue; // Skip own
        ...
        sendto(sockfd, buffer, frame_length, 0, (struct sockaddr*)&socket_address, sizeof(struct sockaddr_ll));
    }
    ...
    while (1) {
        ...
        if (ntohs(recv_eth->h_proto) == ETH_P_ARP) {
            ...
            if (ntohs(recv_arp->opcode) == 2) { // ARP Reply
                ...
                if (gateway_ip && memcmp(recv_arp->sender_ip, gateway_ip, 4) == 0) continue;
                if (memcmp(recv_arp->sender_ip, my_ip, 4) == 0) continue;
                
                // Ensure it's addressed to us
                if (memcmp(recv_arp->target_mac, my_mac, 6) != 0) continue;
                ...
                if (!exists && *out_count < 512) {
                    ...
                    (*out_count)++;
                    printf("[+] Discovered Victim IP: %d.%d.%d.%d MAC: %02x:%02x:%02x:%02x:%02x:%02x\n",
                        recv_arp->sender_ip[0], recv_arp->sender_ip[1], recv_arp->sender_ip[2], recv_arp->sender_ip[3],
